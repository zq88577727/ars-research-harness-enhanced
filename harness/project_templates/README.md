...
`charls_wave_map.template.csv`, `charls_file_manifest.template.csv`, and
...
status, and optional checksums, and the design gate links S1/S2 decisions to the
research question, estimand, temporal ordering, attrition, weight, and claim
language boundaries. The validators check metadata without reading restricted
data unless strict local-readiness modes are explicitly requested.
