...
前序任务复盘：
- 对照上一轮行动建议或阶段目标，逐项说明 complete / partial / blocked。
- 如果存在 partial 或 blocked，不得直接制定新阶段；必须先说明补救动作或等待用户决策。

自动下一步规划：
- 在前序任务 complete 后，提出 1-3 个候选下一步。
- 标明推荐路线、理由、风险、验证方式和是否需要用户确认。
- 下一步规划必须基于本阶段证据，不得跳过未闭合的科研、数据、引用、环境或合规风险。
...
每次完成当前任务或阶段前，必须先回顾前面给出的任务推进建议，并判断这些建议是否已经真正完成。只有当前阶段完整闭合后，才允许进入下一步目标制定。

下一步目标制定不能只回答“做什么”，还要自动规划“为什么做、按什么顺序做、如何验证、哪些边界仍不能跨越”。如果上一阶段没有闭合，下一步行动必须优先补齐上一阶段，而不是开启新阶段。

如果上一阶段因为外部数据授权、人工审核、受限数据下载或本地文件不可用而无法闭合，
必须把该事项登记为 external blocker，说明影响范围、允许继续推进的非依赖工作、
恢复条件和验证命令。不得为了推进阶段而伪造 codebook、原始数据、S3 结果、claim
registry 或投稿产物。
